...
export default function PublicationList() {
  return (
    <ul className="divide-y divide-emerald-800 flex flex-col">
      {publications.map((publication) => (
        <li className="p-4 hover:bg-[#01231c]" key={publication.id}>
          <div className="">
            <div className="">
              <Link
                ...
                className="text-xl font-medium text-emerald-400 hover:underline hover:underline-offset-2 visited:text-purple-600"
              >
                {publication.title}
              </Link>
              <p className="text-lg text-slate-300">{publication.subtitle}</p>
            </div>
